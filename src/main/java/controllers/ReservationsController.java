...
public class ReservationsController {
  ...
  @FXML
  private TableColumn<Reservation, String> colRooms;
  @FXML
  ...
  @FXML
  private TextField txtDetailReservationNumber;
  @FXML
  private TextField txtDetailCustomer;
  @FXML
  private TextField txtDetailCheckIn;
  @FXML
  private TextField txtDetailCheckOut;
  @FXML
  private TextField txtDetailGuests;
  @FXML
  private TextField txtDetailTotalRate;
  @FXML
  private TextField txtDetailStatus;
  @FXML
  ...
  @FXML
  private Button btnReservationConsumptions;
  ...
  public void initialize() {
    // 1. Cargar datos base ANTES de configurar la tabla
    loadCustomers();
    loadStatuses();
    loadReservationRooms(); // ← CRÍTICO: antes de configureTable()

    // 2. Configurar la tabla (ya puede resolver las habitaciones)
    configureTable();
    ...
    // 4. Cargar reservas y aplicar filtros
    loadReservations();
    applyFilters();

    // 5. Selección y estado inicial del detalle
    configureSelection();
    clearDetail();
  ...
  private void applyFilters() {
    ...
    for (Reservation reservation : todasLasReservas) {
      ...
      String customer = getCustomerName(reservation.getIdCustomer()).toLowerCase();
  ...
  private void handleNewReservation() {
    if (dashboardController != null) {
      dashboardController.loadView("/views/NewReservation.fxml");
    } else {
      System.err.println("DashboardController is not connected.");
    }
  }
