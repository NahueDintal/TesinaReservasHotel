...
public class CancellationByMonthController {
  ...
  private void loadData() {
    ...
    statusLabel.setText(String.format("Showing %d month(s). Period: %s -> %s.",
  ...
  private void onExport() {
    ...
    fc.setInitialFileName("cancellation_by_month.csv");
    File file = fc.showSaveDialog(exportButton.getScene().getWindow());
    if (file == null)
      return;

    try (PrintWriter pw = new PrintWriter(file)) {
      pw.println("Month;Total reservations;Cancelled;Probability;95% CI lower;95% CI upper");
      for (Probability p : currentData) {
