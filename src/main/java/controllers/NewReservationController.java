...
public class NewReservationController {
  ...
  /**
   * Bandera para suprimir el recálculo mientras se carga una reserva existente
   * (evita pisar el totalRate guardado en la BD).
   */
  ...
  public void initialize() {
    ...
    loadRoomCards(); // mostrará placeholder hasta que haya huéspedes
    ...
    // ------------------------------------------------------------
    // LISTENERS DE RECÁLCULO (una sola vez)
    // ------------------------------------------------------------
  ...
  public void setReservationToEdit(Reservation reservation) {
    ...
    // Ya terminó de cargar → habilitar recálculos
    loadingReservation = false;
  }
  ...
  private void loadRoomCards() {
    ...
    if (requested <= 0) {
      Label placeholder = new Label("Ingresá la cantidad de huéspedes para ver las habitaciones disponibles.");
    ...
    if (availableRooms.isEmpty()) {
      Label placeholder = new Label("No hay habitaciones disponibles para esa cantidad de huéspedes.");
      placeholder.setStyle("-fx-text-fill: #c0392b; -fx-font-style: italic; -fx-font-size: 13;");
      roomsContainer.getChildren().add(placeholder);
      return;
    }

    // Números de las habitaciones ya seleccionadas (comparación robusta)
    ...
    for (Room room : availableRooms) {
      ...
      // ✅ Comparación por número, no por referencia
  ...
  // ============================================================
  // ROOMS — FILTRADO POR FECHA + CAPACIDAD
  // ============================================================
  private void loadAvailableRooms() {

    // Sin cantidad de huéspedes válida → sin habitaciones
    ...
    // Sin fechas completas → filtramos solo por capacidad
    if (dpCheckIn.getValue() == null || dpCheckOut.getValue() == null) {
      availableRooms.setAll(filterByCapacity(activeRooms));
      loadRoomCards();
      return;
    }

    // Rango inválido → no tocar
    ...
    for (Room room : activeRooms) {
      ...
      boolean alreadyPicked = selectedRooms.contains(room);
  ...
  private List<Room> filterByCapacity(List<Room> rooms) {
  ...
  private int getRequestedGuests() {
    try {
      int n = Integer.parseInt(txtNumberOfGuests.getText().trim());
      return n > 0 ? n : 0;
    } catch (Exception e) {
      return 0;
    }
  ...
  // ============================================================
  /**
   * Recalcula `txtTotalRate` = suma(precios de habitaciones) × noches.
   * - No pisa lo que el recepcionista escribió manualmente si el campo tiene
   * foco.
   * - No recalcula durante la carga inicial de una reserva existente.
   */
  private void updateTotalRate() {
    ...
    if (selectedRooms.isEmpty()) {
      return; // no tocamos nada si no hay habitaciones
    }

    double pricePerNight = selectedRooms.stream()
  ...
  private void configureCustomerSearch() {
    ...
    txtCustomerSearch.focusedProperty().addListener((observable, oldValue, focused) -> {
      if (focused) {
  ...
  private void loadPaymentMethods() {
    try {
      List<PaymentMethod> methods = paymentMethodRepo.getPaymentMethods();
  ...
  private void loadPaymentStatuses() {
    try {
      List<PaymentStatus> statuses = paymentStatusRepo.getPaymentStatuses();
  ...
  private void startConsumptionEdit(Consumption consumption) {
    ...
    cmbConsumptionType.setValue(consumption.getIdConsumptionType() == 1 ? "Producto" : "Servicio");
    txtConsumptionQuantity.setText(String.valueOf(consumption.getQuantity()));
  ...
  private void handleAddConsumption() {
    ...
    Consumption consumption = new Consumption(
        0, consumptionType, idProduct, idService,
  ...
  private void handleModifyConsumption() {
    ...
    if (consumptionBeingEdited != null) {
      ...
      showAlert(Alert.AlertType.INFORMATION, "Consumo",
  ...
  private void handleDeleteConsumption() {
    Consumption selected = tblConsumptions.getSelectionModel().getSelectedItem();
    if (selected == null) {
      showAlert(Alert.AlertType.WARNING, "Consumo", "Seleccione un consumo para anular.");
      return;
    }
    Alert confirmation = new Alert(Alert.AlertType.CONFIRMATION);
    confirmation.setTitle("Anular consumo");
    ...
    try {
      ...
      if (guestsText.isEmpty()) {
        ...
        try {
          ...
          if (numberOfGuests <= 0)
            errors.add("La cantidad de huéspedes debe ser mayor que 0.");
        } catch (NumberFormatException e) {
      ...
      if (reservationStatus == null)
        errors.add("Debe seleccionar el estado de la reserva.");

      ReservationType reservationType = cmbReservationType.getValue();
      if (reservationType == null)
        errors.add("Debe seleccionar el tipo de reserva.");

      String reservationObservations = txtReservationObservations.getText();
      if (reservationObservations != null && reservationObservations.trim().isEmpty()) {
      ...
      if (!paymentText.isEmpty()) {
        try {
          ...
          if (paymentAmount.compareTo(BigDecimal.ZERO) <= 0) {
            errors.add("El importe del pago debe ser mayor que 0.");
          }
        } catch (NumberFormatException e) {
        ...
        if (paymentStatus == null)
          errors.add("Debe seleccionar el estado del pago.");

        paymentObservations = txtPaymentObservations.getText();
        if (paymentObservations != null && paymentObservations.trim().isEmpty()) {
      ...
      if (idReservation <= 0) {
        conn.rollback();
        showError(reservationToEdit == null
      ...
      for (Room room : selectedRooms) {
        ReservationRoom reservationRoom = new ReservationRoom(idReservation, room.getNumber());
  ...
  private void handleBack() {
    if (dashboardController != null) {
      ...
      System.err.println("DashboardController no está conectado.");
    }
  }
