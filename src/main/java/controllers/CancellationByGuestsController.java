...
public class CancellationByGuestsController {
  ...
  @FXML
  private CategoryAxis guestsAxis;
  @FXML
  ...
  @FXML
  private TableColumn<Probability, String> guestsColumn;
  @FXML
  ...
  private void configureTable() {
    guestsColumn.setCellValueFactory(d -> new ReadOnlyObjectWrapper<>(d.getValue().getCategory()));
  ...
  private void loadData() {
    ...
    List<Probability> data = probabilityDAO.findByGuests(from, to, channel);
    currentData.setAll(data);

    updateKpis();
    updateChart();

    statusLabel.setText(String.format("Mostrando %d grupo(s). Período: %s → %s.",
  ...
  private void updateChart() {
    ...
    guestsChart.getData().add(series);
  ...
  private void onExport() {
    ...
    fc.setInitialFileName("cancelaciones_por_huespedes.csv");
    File file = fc.showSaveDialog(exportButton.getScene().getWindow());
    if (file == null)
      return;

    try (PrintWriter pw = new PrintWriter(file)) {
      pw.println("Huéspedes;Reservas;Canceladas;Probabilidad;IC 95% inferior;IC 95% superior");
      for (Probability p : currentData) {
